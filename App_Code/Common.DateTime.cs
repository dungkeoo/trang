...
namespace Commons
{
    public static partial class Common
    {
        ...
        ///// <summary>
        ///// Tính số ngày trong khoảng từ ngày, đến ngày, không tính T7, CN và các ngày lễ (Giáo dục)
        ///// </summary>
        ///// <param name="TuNgay"></param>
        ///// <param name="DenNgay"></param>
        ///// <param name="session"></param>
        ///// <returns></returns>
        //public static int GetDayNumberSubtrackHoliday_GiaoDuc(this DateTime TuNgay, DateTime DenNgay, bool ChinhKhoaThu7, Session session)
        //{
        //    int SoNgay = 0;
        //    DateTime ngay;
        //    if (DenNgay.CompareTo(DateTime.MinValue) != 0 & TuNgay.CompareTo(DateTime.MinValue) != 0)
        //    {
        //        using (XPCollection<NgayNghiTrongNam> list = new XPCollection<NgayNghiTrongNam>(session, CriteriaOperator.Parse("GetYear(NgayNghi) = ? and CongTy = ?", TuNgay.Year, Common.CongTy(session).Oid)))
        //        {
        ...
        //            }
        //            for (ngay = TuNgay.Date; ngay <= DenNgay.Date; ngay = ngay.AddDays(1))
        //            {
        //                if (ngay.DayOfWeek != DayOfWeek.Sunday 
        //                    && !NgayNghiTrongNamList.Contains(ngay.Date)
        //                    && ((ChinhKhoaThu7 != true && ngay.DayOfWeek != DayOfWeek.Saturday)
        //                        || ChinhKhoaThu7 == true))
        //                {
        //                    SoNgay++;
        //                }
        ...
        /// <param name="session"></param>
        /// <returns></returns>
        public static int GetDayNumberSubtrackWeekend_DaiHoc(this DateTime TuNgay, DateTime DenNgay)
        {
            int SoNgay = 0;
            if (DenNgay.CompareTo(DateTime.MinValue) != 0 & TuNgay.CompareTo(DateTime.MinValue) != 0)
            {
        ...
        /// <param name="session"></param>
        /// <returns></returns>
        public static decimal GetDayNumberSubtrackWeekend_ManNonAndPhoThong(this DateTime TuNgay, DateTime DenNgay)
        {
            decimal SoNgay = 0;
            if (DenNgay.CompareTo(DateTime.MinValue) != 0 & TuNgay.CompareTo(DateTime.MinValue) != 0)
            {
